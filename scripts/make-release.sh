...
# Флаг -a заставляет git создать аннотированный тег
# --cleanup=whitespace отключает удаление строк, начинающихся с `#` (чтобы вы могли использовать заголовки Markdown)
git tag -a "$NEXT_TAG" --cleanup=whitespace
