...
*   **transparent_listen**: (Опционально, только Linux) Порт для прозрачного проксирования. Используется при запуске `pp-client start --full-tunnel`; если не задан, CLI подставляет `127.0.0.1:1090`.
...
Для работы GeoIP/GeoSite требуются файлы `geoip.dat` и `geosite.dat` в директории `data/` (устанавливаются автоматически установщиком или отдельными скриптами `install-geo`).
